...
static void config_sysrst(const dif_pwrmgr_t *pwrmgr) {
  ...
  CHECK_DIF_OK(dif_pwrmgr_set_request_sources(pwrmgr, kDifPwrmgrReqTypeReset,
                                              ...
                                              kDifToggleEnabled));
  // 100 us for enough waiting window
  uint32_t wait_us =
      udiv64_slow(500 * 1000000 + kClockFreqAonHz - 1, kClockFreqAonHz, NULL);
...
bool test_main(void) {
  ...
  if (rst_info == kDifRstmgrResetInfoPor) {
    ...
    sleep_wdog_bite_test(&aon_timer, &pwrmgr, /*bark_time_us=*/200);
             ...
             (kDifRstmgrResetInfoWatchdog | kDifRstmgrResetInfoLowPowerExit)) {
    LOG_INFO("Booting for the fourth time due to wdog bite reset during sleep");
  }
